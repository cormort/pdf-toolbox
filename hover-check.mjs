...
if (!target) { console.error('Edge 的 debugging port 沒起來'); cleanup(2); }
...
const focus = await evaluate(`(() => {
  ...
  return { tag: el.tagName.toLowerCase() + (el.id ? '#' + el.id : ''), width: s.outlineWidth, style: s.outlineStyle, color: s.outlineColor };
})()`);
if (!focus) {
  check('Tab 之後要有元素取得焦點', '沒有', '元素');
} else {
  console.log(`  · 焦點在 ${focus.tag}`);
