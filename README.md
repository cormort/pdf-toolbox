...
- [ ] OCR（可攜 Tesseract + 繁中語言檔）
- [ ] 工具間傳檔（例如比對結果送 CMYK），有需要再做
- [x] exe 圖示（`web/icons/` ＋ `rsrc_windows_amd64.syso`）；版本資訊還沒做

第三批（照 2026-09-26「棣南 PDF 文電通專業版」評測的功能清單補的；出處
<https://www.kocpc.com.tw/archives/522096>。文中主打三項：就地編輯 PDF 文字、簽署、內建轉檔）：
- [ ] 簽名工具：手繪／輸入／圖片三種簽名，可存多個、拖到任一頁任意位置，顏色／線寬／透明度可調
      （pdf-lib 疊圖，與 pdf_recompose 同一套；先做這項）
- [ ] 標記密文（真正的永久刪除）：拉框選取後移除敏感內容。優先用內容流過濾，只拿掉框內文字／圖片，
      其餘文字仍可搜尋；驗收標準是用 `pdftotext` 確認被遮的字抽不到。過濾做不到的檔案再退回
      「整頁點陣化＋黑框」。（文章強調：用黑色方塊蓋掉不等於刪除，被遮的內容仍可複製）
- [ ] 數位簽章：pdfcpu v0.15 `api.Sign`，匯入 `.p12`／PEM 或產生自簽憑證；輸出後顯示簽章者與簽章時間
- [ ] 填寫 PDF 表單（pdfcpu form：list／fill／flatten），可匯出欄位值
- [ ] PDF 管理員（可攜版專屬）：掃描指定資料夾，列出檔名／頁數／大小／標題並支援關鍵字搜尋，
      一鍵把檔案送進其他分頁（閱讀、頁面重組、壓縮、比對…）
- [ ] viewer 註解補「圖說文字」（方塊＋箭頭）與形狀工具（箭頭／線／方框／圓）
      （醒目提示、附註、手繪、螢光筆已有）
- [ ] 就地編輯 PDF 文字（文章主打）：中文要重寫字型與內容流，純本機工具風險高；先只評估
      「白底覆蓋＋新文字」的標示性編輯，不承諾改回原字型
- [ ] PDF→Word／Excel／PPT 保版型轉檔（文章主打）：要綁 LibreOffice（zip 多約 400–500MB）
      或自寫重排，品質有限，與 OCR 一起最後評估

文章列出、本工具箱已具備的：PDF 合併（頁面重組）、減肥（壓縮 PDF）、一鍵存所有圖片（取出內嵌圖片）、
加密與權限（密碼保護，比文章更細）、醒目提示／附註／繪圖註解（閱讀與註解）、PDF 轉 TXT（PDF 轉文字）。
